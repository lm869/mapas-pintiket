    ...
    b-container(fluid)
      b-row
        b-col(cols="9")
          ...
          button(@click="drillUp") Drill Up
        b-col(cols="3")
          h4.text-center asientos seleccionados
</template>
...
export default {
  ...
  methods: {
    ...
    createSeatMap () {
      ...
      map.geoData(this.map)
      seatmap.geoData(this.chairs)
      ...
      console.log('fin añadido')
      seatmap.tooltip().titleFormat('{%seriesName}')
      seatmap.tooltip().format('$ {%value} pesitos')
      seatmap.legend().enabled(true).position('bottom').padding({top: 15})

      map.choropleth(mapInfo)
